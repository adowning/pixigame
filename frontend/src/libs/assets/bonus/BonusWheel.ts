...
export class BonusWheel extends PIXI.Sprite {
  ...
  private build() {
    ...
    this.black
      ...
      .drawRect(0, 0, PAGE_SIZE_DEFAULT.width, PAGE_SIZE_DEFAULT.height)
      .endFill()
    ...
    // Background
    this.back = this.cont.addChild(
      new PIXI.Sprite(PIXI.Texture.from('images/frenzy/bonus/bonus_bg.png')),
    )
    ...
    this.title = this.addChild(
      new PIXI.Sprite(
        PIXI.Texture.from(
          `images/frenzy/bonus/${
            this.state.isDaily ? 'popup_title_daily' : 'popup_title_weekly'
          }.png`,
        ),
      ),
    )
    ...
    // Help Button
    this.help = this.conttitle.addChild(
      new ButtonItem('images/frenzy/bonus/help.png', () => {
    ...
    // Close Button
    this.close = this.data.addChild(
      new ButtonItem('images/frenzy/bonus_close.png', () => {
  ...
  private toggleTab(isDaily: boolean) {
    if (this.state.isDaily !== isDaily) {
      ...
      this.progressPanel.setIsDaily(this.state.isDaily)
      this.title.texture = PIXI.Texture.from(
        `images/frenzy/bonus/popup_title_${this.state.isDaily ? 'daily' : 'weekly'}.png`,
      )
    }
  }
  ...
  private onResize(data: any) {
    ...
    this.black
      .beginFill(0x000000, 0.8)
      .drawRect(0, 0, data.w / data.scale, data.h / data.scale)
      .endFill()
...
class BonusItem extends PIXI.Sprite {
  ...
  constructor() {
    ...
    this.cont = this.addChild(new PIXI.Sprite())
    //emptry bronze
    this.state1 = this.cont.addChild(
      new PIXI.Sprite(PIXI.Texture.from('images/frenzy/bonus/empty_bonus_star.png')),
    )
    //filled bronze
    this.state2 = this.cont.addChild(
      new PIXI.Sprite(PIXI.Texture.from('images/frenzy/bonus/filled_bonus_star.png')),
    )
...
class BonusData extends PIXI.Sprite {
  ...
  constructor() {
    ...
    const back = this.cont.addChild(
      new PIXI.Sprite(PIXI.Texture.from('images/frenzy/bonus/total_coin_bg.png')),
    )
    back.x = -100
    back.y = 120
    back.width = 400
    ...
    const tween = gsap.to(
      {},
      {
        ...
        onUpdate: () => {
          ...
          this.task.text = value.toLocaleString('en-US', {
...
class RemainingTime extends PIXI.Sprite {
  ...
  constructor() {
    ...
    //background image for weekly task 👇
    const back = this.cont.addChild(
      new PIXI.Sprite(PIXI.Texture.from('images/frenzy/bonus/remaining_time_bg.png')),
    )
    back.x = -185
    back.y = -30
    back.width = 600
    ...
    const remainTime = setInterval(() => {
      ...
      this.setDay(
        `${days}D, ${hours < 10 ? '0' + hours : hours}:${minutes < 10 ? '0' + minutes : minutes}:${seconds < 10 ? '0' + seconds : seconds}`,
      )
...
class ProgressPanel extends PIXI.Sprite {
  ...
  constructor() {
    ...
    this.back = new PIXI.Sprite(
      PIXI.Texture.from(
        `images/frenzy/bonus/${this.isDaily ? 'daily' : 'weekly'}_progress_bar.png`,
      ),
    )
    this.back.width = 600
  ...
  updateProgressBar() {
    ...
    // Initialize back as PIXI.Sprite with initial texture
    this.back.texture = PIXI.Texture.from(
      `images/frenzy/bonus/${this.isDaily ? 'daily' : 'weekly'}_progress_bar.png`,
    )
    this.back.width = 600
    this.back.height = 1000
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const status = new PIXI.Sprite(
        PIXI.Texture.from(
          `images/frenzy/bonus/${prog === 10 ? 'btn_completed' : 'btn_inprogress'}.png`,
        ),
      )
      status.x = 350
      status.y = _y - 40
