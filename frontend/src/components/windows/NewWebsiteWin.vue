<template>
  <div className="modal-window-newwebsite">
    <div className="modal-window-newwebsite__scale-cont">
      <img className="modal-window-newwebsite__img" src="/images/frenzy/maintenance.png" alt="" />
      <a href="https://www.finfrenzy.app" className="modal-window-newwebsite__text">
        www.finfrenzy.app
      </a>
      <img
        className="modal-window-newwebsite__close_btn"
        @click="onClose"
        src="/images/frenzy/bonus_close_2.png"
        alt=""
      />
    </div>
  </div>
</template>

<script>
import { EE } from '../../Game'
import '@/assets/css/newwebsite.css'
import { PAGE_SIZE_DEFAULT } from '@/libs/common/Config'

export default {
  props: ['onClose'],
  methods: {
    onResize(data) {
      const cont = document.getElementsByClassName('modal-window-newwebsite__scale-cont')[0]
      const sc = Math.min(data.h / PAGE_SIZE_DEFAULT.height, data.w / PAGE_SIZE_DEFAULT.width)
      if (cont) {
        cont.style.transform = `scale(${sc})`
      }
    },
    onClose() {
      this.$emit('close')
      EE.emit('SHOW_LOGIN')
    },
  },
  mounted() {
    EE.addListener('RESIZE', this.onResize)
    EE.emit('FORCE_RESIZE')
  },
}
</script>
